...
bool validateParameters(int spray_num, int hole_start, int hole_space) {
    if (spray_num <= 0 || spray_num > 0x10000) {
        std::cerr << "[-] Invalid SPRAY_NUM value (must be between 1 and 65536)" << std::endl;
        return false;
    }
    if (hole_start >= spray_num) {
        std::cerr << "[-] HOLE_START must be less than SPRAY_NUM" << std::endl;
        return false;
    }
    if (hole_space <= 0 || hole_space > 0x100) {
        std::cerr << "[-] Invalid HOLE_SPACE value (must be between 1 and 256)" << std::endl;
        return false;
    }
    return true;
}
...
int main(int argc, char *argv[]) {
    ...
    std::cout << COLOR_HEADER << "╔════════════════════════════════╗\n"
          ...
          << "DELAY=" << COLOR_VALUE << corrupt_delay << "s" << COLOR_RESET << "\n"
