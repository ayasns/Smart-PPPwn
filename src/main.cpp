...
bool parsenums(std::string& argstring, int& argnum, int defaultVal) {
    ...
    try {
        if (argstring.size() >= 2 && argstring.compare(0, 2, "0x") == 0) {
            ...
            std::cerr << "[-] Invalid number format: " << argstring << std::endl;
            return false;
        }
        return true;
    } catch (const std::exception& e) {
        std::cerr << COLOR_ERROR << "[-] Error parsing number: " << COLOR_RESET << std::endl;
        return false;
    }
}
...
bool validateParameters(int spray_num, int hole_start, int hole_space) {
    if (spray_num <= 0 || spray_num > 0x10000) {
        std::cerr << COLOR_ERROR << "[-] Invalid SPRAY_NUM value (must be between 1 and 65536)" << COLOR_RESET << std::endl;
        return false;
    }
    if (hole_start >= spray_num) {
        std::cerr << "\033[1;31m[-] HOLE_START must be less than SPRAY_NUM\033[0m" << std::endl;
        return false;
    }
    if (hole_space <= 0 || hole_space > 0x100) {
        std::cerr << "\033[31;5m[-] Invalid HOLE_SPACE value (must be between 1 and 256)\033[0m" << std::endl;
        return false;
    }
    return true;
}
...
╚════════════════════════════════════════╝
    ...
    std::cout << COLOR_HEADER << "╔════════════════════════════════╗\n"
          ...
          << "  " << COLOR_LABEL << "Timing:" << COLOR_RESET << "\n"
          ...
          << "    HOLE_START=" << COLOR_VALUE << hole_start << COLOR_RESET << "  "
          << "HOLE_SPACE=" << COLOR_VALUE << hole_space << COLOR_RESET << "\n"
          << std::endl;
